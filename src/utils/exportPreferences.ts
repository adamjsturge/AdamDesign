...
export const fontFamilies = {
    system: {
        name: "System",
        description: "Native system fonts for fastest loading and platform consistency",
    ...
    opendyslexic: {
        name: "OpenDyslexic",
        description: "Accessibility-focused font designed for readers with dyslexia",
...
export const typographyScales = {
    tight: {
        name: "Tight",
        description: "Minimal size differences between heading levels, information-dense",
...
export const shadowStyles = {
    none: {
        name: "None",
        description: "No shadows - use background color contrast for visual hierarchy",
        value: "none",
...
export const animationStyles = {
    ...
    minimal: {
        name: "Minimal",
        description: "Quick, subtle transitions (recommended for productivity apps)",
        duration: "100ms",
...
export function generateMarkdown(preferences: Record<string, string>): string {
    const colorScheme = colorSchemes[preferences.colorScheme as keyof typeof colorSchemes] ?? colorSchemes.dark;
    const accent = accentColors[preferences.accentColor as keyof typeof accentColors] ?? accentColors.blue;
    const font = fontFamilies[preferences.fontFamily as keyof typeof fontFamilies] ?? fontFamilies.system;
    const typography = typographyScales[preferences.typographyScale as keyof typeof typographyScales] ?? typographyScales.tight;
    const spacing = spacingDensities[preferences.spacingDensity as keyof typeof spacingDensities] ?? spacingDensities.compact;
    const radius = borderRadii[preferences.borderRadius as keyof typeof borderRadii] ?? borderRadii.sm;
    const shadow = shadowStyles[preferences.shadowStyle as keyof typeof shadowStyles] ?? shadowStyles.none;
    const animation = animationStyles[preferences.animationStyle as keyof typeof animationStyles] ?? animationStyles.minimal;
    const button = buttonStyles[preferences.buttonStyle as keyof typeof buttonStyles] ?? buttonStyles.filled;
    const input = inputStyles[preferences.inputStyle as keyof typeof inputStyles] ?? inputStyles.filled;
    const container = containerWidths[preferences.containerWidth as keyof typeof containerWidths] ?? containerWidths.wide;
    ...
    const sections = [
        ...
        font.import ? `- **Import**: \`<link href="${font.import}" rel="stylesheet">\`` : `- **Import**: None required (system fonts)`,
        ``,
        ...
        shadow.name === "None" ? `- **Alternative**: Use background color differences for hierarchy` : ``,
        ``,
